resource "null_resource" "create_warehouse_bucket" {
  ...
  triggers = {
    ...
    bucket_name   = var.s3_warehouse_bucket_name
  }
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      for i in 1 2 3; do
        if kubectl exec -n ${var.kubernetes_storage_namespace} $POD_NAME -c minio -- sh -c 'mc alias set local http://localhost:9000 ${var.s3_access_key} ${var.s3_secret_key} && mc mb --ignore-existing local/${var.s3_warehouse_bucket_name}'; then
      ...
      echo "Failed to create bucket after 3 attempts"
      exit 1
    EOT
  }
}